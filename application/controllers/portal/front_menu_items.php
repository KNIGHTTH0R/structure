...
class Front_menu_items extends CI_Controller {
	function __construct() {
		parent::__construct();
		$this->load->model( 'admin/menu_items_model' );
		$this->js = 'assets/js/front_menu_items_crud_init.js';
	}
	
	public function index() {
		$params['page_title']		= 'Menu Item';
		$params['page_header']	= 'Menu Item <small>list</small>';
		$params['scripts']			= array( 'js' => $this->js, 'init' => 'Front_menu_items_list' );
		
		$menu_item_list = array(
			'actions' => array(
				'href' => 'front_menu_items/create',
				'icon_text' => 'Create',
				'reorder' => TRUE
			)
		);
		...
		$params['list']						= $this->menu_items_model->menu_item_list( 'fr' );
		
		$this->layout->view( 'portal/front_menu_items_view', $params );
	}
	
	public function create() {
		$params['page_title']		= 'Menu Item';
		$params['page_header']	= 'Menu Item <small>create</small>';
		$params['scripts']			= array( 'js' => $this->js, 'init' => 'Front_menu_items_create' );
		

		$params['parent_options']			= $this->menu_items_model->parent_options();
		$params['site_url']						= site_url();
		
		$this->layout->view( 'portal/front_menu_items_create_view', $params );
	}
	
	public function insert() {
		$menu_item = $this->menu_items_model->menu_item_insert();
		if( $menu_item ) {
			echo 's|created Menu Item|' . site_url( 'portal/front_menu_items' );
		} else {
	...
	public function revise( $menu_item_id ) {
		...
		$params['scripts']						= array( 'js' => $this->js, 'init' => 'Front_menu_items_revise' );
		
		$params['menu_item']					= $this->menu_items_model->get_menu_item( $menu_item_id );
		$params['parent_options']			= $this->menu_items_model->parent_options();
		
		$this->layout->view( 'portal/front_menu_items_revise_view', $params );
	}
	
	public function update() {
		$menu_item = $this->menu_items_model->menu_item_update();
		if( $menu_item ) {
			echo 's|updated Menu Item|' . site_url( 'portal/front_menu_items' );
		} else {
