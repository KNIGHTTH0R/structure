...
function gen_ui_framework_block( columns, num_columns, count ) {
	...
	for( var i = 0; i < num_columns; i++ ) {
		...
		// Fix this ugly shit
		var widget_input 	= '<div class="form-group"><label class="control-label col-md-3">Allow Widgets?</label><div class="col-md-9"><input type="checkbox" class="make-switch" data-size="small" name="allow_widgets[' + count + '][' + column_count + ']" value="y" checked /></div></div>';
		var width_input		= '<div class="form-group"><label class="control-label col-md-3">Column Width</label><div class="col-md-9"><input class="form-control input-small" type="text" name="framework_width[' + count + '][' + column_count + ']" value="' + columns + '" /></div></div>';
...
function gen_ui_object_params_options() {
	...
	var field_name_input		= form_group.clone().append( $( '<input>', { type: 'text', class: 'form-control object-params params-field_name', placeholder: 'Field Name' } ) );
	...
	var remove_button					= $( '<button>', { type: 'button', class: 'btn red remove-params', text: 'Remove' } );
	
	var column_1						= $( '<div>', { class: 'col-md-4' } ).append( select );
	var column_2						= $( '<div>', { class: 'col-md-3' } ).append( label_input );
	var column_3						= $( '<div>', { class: 'col-md-3' } ).append( field_name_input );
	var column_4						= $( '<div>', { class: 'col-md-2' } ).append( remove_button );
